...
$response = Invoke-RestMethod "$URL_DEVOPS_COMPLETE/_apis/release/definitions?`$expand=Environments,Artifacts,Variables,lastRelease,tags,triggers&`$top=100&api-version=5.1&searchText=$filtreProject" -Method 'GET' -Headers @{Authorization = ("Basic {0}" -f $base64AuthInfo)}
...
if($response.value.Count -gt 0){
    foreach ($releaseDefinition in $response.value  | Sort-Object -Property name){
        if( $filtreProject -eq "" -or $releaseDefinition.name -like "*$filtreProject*"){
            ...
            if( $flagShowEnvironments -or $flagShowDeployPhases){
                ...
                $responseDeployPhases = Invoke-RestMethod "$URL_DEVOPS_COMPLETE/_apis/release/definitions/$($releaseDefinitionID)?api-version=5.1" -Method 'GET' -Headers @{Authorization = ("Basic {0}" -f $base64AuthInfo)}
    ...
    write-host "No available releases" -ForegroundColor Magenta
}
