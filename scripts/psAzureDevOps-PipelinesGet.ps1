...
$filtreProject = "*Results.Dev.Continuous*"
$filtreProject = "*esults.RUN_Repli_Alim_Process-DEV*"
#$flagShowVariables = $true
$toDebug = $false
$toDebug = $false
#$flagRunPipeline_id = 455 #Results.Dev.Continuous(#455)
#$flagRunPipeline_id = 2090 #[ClientPortal] Results.RUN_Repli_Alim_Process-DEV(#2090)
...
$url = "$URL_DEVOPS_COLLECTION/_apis/projects?api-version=6.0"
if($toDebug){write-host "fetching $url ..." -ForegroundColor Yellow}
$getprojects = Invoke-RestMethod $url -Method 'GET' -Headers @{Authorization = ("Basic {0}" -f $base64AuthInfo)}
if( $getprojects.value.Count -gt 0){
    ...
    foreach ($proj in $getprojects.value){
        ...
        $url = "$URL_DEVOPS_COLLECTION/$($proj.name)/_apis/build/definitions?api-version=6.0&includeAllProperties=true&searchText=$filtreProject"
        if($toDebug){write-host "fetching $url ..." -ForegroundColor Yellow}
        $response = Invoke-RestMethod $url -Method 'GET' -Headers @{Authorization = ("Basic {0}" -f $base64AuthInfo)}
        ...
        foreach ($pipeline in $response.value | Sort-Object {$_.name}){
            ...
            $strpipeline = "[$($proj.name)] "+ $pipeline.name + "(#$pipelineID)"
            ...
            if($flagRunPipeline_id -gt 0 -and $pipelineID -eq $flagRunPipeline_id){
                write-host "   démarrage du pipeline ... $pipelineID" -ForegroundColor Green
                $body = "{}"
                $responePipelineRunExecute = Invoke-RestMethod "$URL_DEVOPS_COLLECTION/$($proj.name)/_apis/pipelines/$($pipelineID)/runs?api-version=6.0-preview.1" -Method 'POST' -body $body -ContentType "application/json" -Headers @{Authorization = ("Basic {0}" -f $base64AuthInfo)}    
                $responePipelineRunExecute
            }
        
            #recup des deployPhases definition
            if( $flagShowDeployPhases -eq $true -or $flagShowDeployPhasesDetails -eq $true){
                $responsePipelineRuns = Invoke-RestMethod "$URL_DEVOPS_COLLECTION/$($proj.name)/_apis/pipelines/$($pipelineID)/runs?api-version=6.0-preview.1" -Method 'GET' -Headers @{Authorization = ("Basic {0}" -f $base64AuthInfo)}
                ...
                foreach($php in $pkgInfo.process.phases ){
                    ...
                    foreach($phpS in $php.steps){
                        ...
                        if($phpS.task.definitionType -eq "metaTask"){
                            ...
                            $tasksFromMetaTask = Invoke-RestMethod "$URL_DEVOPS_COLLECTION/$($proj.name)/_apis/distributedtask/taskgroups/$($phpS.task.id)?api-version=6.0-preview.1&expanded=True" -Method 'GET' -Headers @{Authorization = ("Basic {0}" -f $base64AuthInfo)}
